...
server <- function(input, output, session){
    ...
    bestSoFar <- reactiveValues(ssr = NULL, # best guess for SSR value so far
                                ...
                                b2G = NULL) # z slope corresponding to best guess so far
    
    # Disabling (actual) dataGen button, once advanced to lambda panel
    ...
    all <- eventReactive(input$dataGenButton, {
        ...
        # Show download button, hide "NOTE:..." on Data page
        shinyjs::show("downloadData")
        shinyjs::hide(selector="h4.simFyiHdr")
        
        # Swap long lambda stuff with shorter
        shinyjs::hide("lambda")
        shinyjs::show("lamVal")
        shinyjs::hide("lamInstr")
        shinyjs::hide("dataGenButton")
                
        # Begin
        dat <- dataGen()[[1]]
        
        mod <- glmnet(as.matrix(dat[,2:3]), dat[,1], family="gaussian", 
                      standardize=FALSE, lambda=input$lambda)  # not standardizing here b/c already did it elsewhere
        
        
        # Pull truth
        coeffs <- as.matrix(coef(mod))           # betas
        truth  <- sum(                           # true penalized SSR
                    (dat$y - (coeffs[2] * dat$x1 + coeffs[3]*dat$x2)) ^ 2
                  ) + 
                  input$lambda * sum(abs(coeffs[-1]))  
    ...
    dataGen <- function() {
        ...
        b2HatTrue <- sample(-20:20, 1)/4	  # for z
        noise <- 1.5		                  # how spread out the u_i's are 
            
        # x: continuous
        x1 <- rnorm(input$nObs)
        
        # z: discrete (to help with the graphs, or else I'll go nuts)
        x2 <- rbinom(input$nObs, 1, 0.6)
        
        e <- rnorm(input$nObs)
        y <- aHatTrue + b1HatTrue*x1 + b2HatTrue*x2 + noise*e 
        
        # STANDARDIZE/DEMEAN
        ## STANDARDIZE Y
        yStd  <- (y - mean(y))/ sdpop(y)  # Hastie recommends N, not N-1, in glmnet tutorial.
        
        ## STANDARDIZE Xs
        x1Std <- (x1 - mean(x1))/ sdpop(x1)  
        x2Std <- (x2 - mean(x2))/ sdpop(x2)  
                                            
        # Return both dfs, JIC
        return(list( data.frame(y=yStd, x1=x1Std, x2=x2Std),
                     data.frame(y, x1, x2)
                   )
               )
    }
    ...
    obsSSR <- reactive({
        xb <- (input$b1Hat*all()[[1]]$x1 + input$b2Hat*all()[[1]]$x2)
        obs <- (all()[[1]]$y - xb)^2
        pen <- input$lambda *(abs(input$b1Hat) + abs(input$b2Hat))

        c(sum(obs)+pen, data.frame(obs), pen)
    })  
...
#***********************************************************************      
    ...
    output$eq_fullSSR <- renderUI({
        ...
        linComb <- paste0('y_i', ifelse(input$b1Hat>=0, "-", "+"), abs(input$b1Hat), 'x_i', 
                                 ifelse(input$b2Hat>=0, "-", "+"), abs(input$b2Hat), 'z_i')
        
        linCombOrig <- paste0('y_i - \\left(', ifelse(input$b1Hat>=0, "+", "-"), abs(input$b1Hat), 'x_i',
                                               ifelse(input$b2Hat>=0, "+", "-"), abs(input$b2Hat), 'z_i \\right)')
       
        pen <- paste0(input$lambda, "\\left( \\lvert", input$b1Hat, "\\rvert + \\lvert", input$b2Hat, "\\rvert \\right)")
        
        # For first line of generic, do underbraces to highlight OLS part + LASSO part
        withMathJax(
            paste(
                '\\(\\begin{align}
                
     \\text{Penalized } \\sum \\hat{u}^2 &= \\underbrace{
                                                \\sum\\limits_{i=1}^{', input$nObs, '} \\left\\{ \\left(y - \\hat{y}      \\right) ^2  \\right\\} 
                                            }_{\\text{regular OLS}} + 
                                            \\underbrace{ ', pen, ' \\vphantom{\\sum\\limits_{i=1}^{', input$nObs, '}}
                                            }_{\\text{LASSO penalty}} \\\\
                                         &= \\sum\\limits_{i=1}^{', input$nObs, '} \\left\\{ \\left( ', linCombOrig, '\\right) ^2  \\right\\} + ', pen, ' \\\\
                                         &= \\sum\\limits_{i=1}^{', input$nObs, '} \\left\\{ \\left( ', linComb,     '\\right) ^2  \\right\\} + ', pen, ' 
                    \\end{align}
                \\)'
            )
        )
    ...
    output$eq_fullSSR_spec <- renderUI({
        ...
        for(i in 1:input$nObs){
            ...
            linComb <- paste0(round(all()[[1]][i,1], 3), ifelse(input$b1Hat>=0, "-", "+"), abs(input$b1Hat), '*', round(all()[[1]][i,2], 3),
                                                         ifelse(input$b2Hat>=0, "-", "+"), abs(input$b2Hat), '*', round(all()[[1]][i,3], 3))
            
            # get proper sign depending on whether the DV for this obsv is a 0 or 1 with ifelses
            equation <- paste0(equation, ifelse(i!=0, "& \\phantom{=} {} +", ""), ' \\left. \\left( ',  
        ...
        equation <- paste0(equation, ' \\text{Penalized } \\sum \\hat{u}^2 &=', ssr)
    ...
    # LAMBDA VALUE ====
    output$lamVal <- renderUI({
        input$dataGenButton
        withMathJax(
            paste0("\\(\\lambda = ", input$lambda, "\\)")
        )
    })
    
    # MAIN GRAPH: Z=0 ====
    output$gph0<- renderPlot({    
        # Get dataset
        dat <- all()[[1]] %>% filter(x2==min(x2)) # can't hardcode as 0 b/c of the standardization
        
        # to force the plot to have stableish dimensions w/some cushion
        yRng <- cushion(dat$y , .2)
        xRng <- cushion(dat$x1, .2)
        
        # get four corners
        yHat <- input$b1Hat*dat$x1 + input$b2Hat*dat$x2 
        res <- dat$y - yHat
        
        
        yMax <- apply(cbind(dat$y, yHat), 1, max)  
        yMin <- apply(cbind(dat$y, yHat), 1, min) 
        xMax <- apply(cbind(dat$x1, (dat$x1 - res)), 1, max)   
        xMin <- apply(cbind(dat$x1, (dat$x1 - res)), 1, min)   
        ...
        gg<- ggplot(data=dat) +
                ...
                geom_abline(slope = input$b1Hat, intercept = input$b2Hat*val, col = "red") +
    ...
    # MAIN GRAPH: Z=1 ====
    output$gph1<- renderPlot({      
        # Get dataset
        dat <- all()[[1]] %>% filter(x2==max(x2)) # can't hardcode as 1 b/c of the standardization
    ...
    output$sgzTable <- renderUI({
        ...
        # Get fake regression table to then override
        fake <- lm(y ~ 0 + x1 + x2, data=all()[[1]] )
        
        # get the raw HTML
        tab <- stargazer(fake, type="html",
                    covariate.labels = c("<em>x</em>", "<em>z</em>"),
                    coef = list(c(coef(all()[[2]])[2], coef(all()[[2]])[3])),
                    apply.se = function(x) { 0 },
                    title="Model Results",
                    dep.var.labels  = "DV: <em>y</em>",
                    dep.var.caption = "",
                    add.lines = list(c("<em>&lambda;</em>", input$lambda),
                                     c("Penalized SSR", 
                                           round(all()[[3]], places))
                                    ),
                    keep.stat = c("N"),
                    report="vc",               # just for you, Neal!
                    omit.table.layout="n",
                    digits=places,
                    digits.extra=4
            )
    ...
    # C. GUESS' LASSO PENALTY VALUE ====
    output$lassoPenTerm <- renderUI({
        withMathJax(
            paste0('\\(', input$lambda * sum(abs(input$b1Hat)+abs(input$b2Hat)), "\\)") 
        )
    })
    ...
    output$trueEsts <- renderUI({
        ...
        coeffs <- as.matrix(coef(all()[[2]]))
        
        withMathJax(
            paste0('\\( 
                        \\beta_1 =', round(coeffs[2], 3), 
                     ',~\\beta_2 =', round(coeffs[3], 3), 
                   '\\)')
        )
    })
    ...
    output$data_table <- DT::renderDataTable({
        ...
        if(input$dataGenButton>0){
            ...
            sketch <- htmltools::withTags(table(
                ...
                tfoot(
                    tr(
                        th(colspan="5", class="dt-center dt-right", paste0("SSR (across all ", n, " observations): ", round(sum(dat$uHatSq),3)))
                    ),
                    tr(
                        th(colspan="5", style="border: 0px solid black;",
                           class="dt-center dt-right", paste0("LASSO Penalty: ", round(obsSSR()[[3]],3)))
                    ),
                    tr(
                        th(colspan="5",  style="border: 0px solid black; background-color:#f4f0e8;",
                           class="dt-center dt-right", paste0("TOTAL: ", round(sum(dat$uHatSq)+obsSSR()[[3]],3)))
                    )
...
#***********************************************************************     
    ...
    observeEvent(input$solnButton, {
        ...
        b1Min <- ifelse(coeffs[2]< -6, floor(coeffs[2]), -6)
        b1Max <- ifelse(coeffs[2]>  6, ceiling(coeffs[2]),  6)
            
        b2Min <- ifelse(coeffs[3]< -6, floor(coeffs[3]), -6)
        b2Max <- ifelse(coeffs[3]>  6, ceiling(coeffs[3]),  6)
        
        updateSliderInput(session, "b1Hat", value=list(coeffs[2]),
                          min=list(b1Min), max=list(b1Max) )
        updateSliderInput(session, "b2Hat", value=list(coeffs[3]),
                          min=list(b2Min), max=list(b2Max) )
    ...
    observeEvent(input$restoreButton, {
        ...
        updateSliderInput(session, "b2Hat", value=list(bestSoFar$b2G))
    })
